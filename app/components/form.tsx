...
export default function AddCarForm(props: any) {
    ...
    const {mutate} = useMutation(
        async (formData: { title: string, nameRenter: string, firstNameRenter: string, descriptionCar: string, emailRenter: string }) => {
            const { title, nameRenter, firstNameRenter, descriptionCar, emailRenter } = formData;
            await axios.post('/api/cars/addCars', { title, nameRenter, firstNameRenter, descriptionCar, emailRenter })
        },
        {
            onError: (error) =>{
    ...
    return (
        ...
        <form onSubmit={submitCar} className='bg-white m-8 p-8 rounded-md w-full max-w-lg'>
            ...
            <p className={`font-bold text-sm ${descriptionCar.length > 300 ? "text-red-700" : "text-black"}`}>
